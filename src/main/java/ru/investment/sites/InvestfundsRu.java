...
public class InvestfundsRu extends AbstractSite {
    ...
    public ShareDTO task() {
        buildUrl(SOURCE_PRE + getDto().getTicket().toUpperCase() + SOURCE_POST);
        Document doc;
        ...
        if (tree.path("total").intValue() == 0) {
            log.warn(getDto().getSource() + " не нашла " + getDto().getTicket());
            return null;
        }
        if (tree.path("currentCount").intValue() > 0) {
            boolean tickerExists = tree.path("currentResults").findPath("trading_grounds").findPath("ticker").textValue().equalsIgnoreCase(getDto().getTicket());
            if (!tickerExists) {
                tickerExists = tree.path("currentResults").findValues("ticker").stream().filter(s -> s.textValue().equalsIgnoreCase(getDto().getTicket())).findAny().isPresent();
            }
            if (tickerExists) {
                int count = 0, index = 0;
                for (String ticker : tree.findValuesAsText("ticker")) {
                    if (ticker.equalsIgnoreCase(getDto().getTicket())) {
                        count++;
                    }
                }
                if (count > 1 && tree.path("currentCount").intValue() > 1) {
                    if (tree.path("total").intValue() == tree.path("currentCount").intValue()) {
                        log.error("Более одного типа '" + getDto().getTicket() + "': " + count + " шт. " +
                ...
                while (tel.hasNext()) {
                    ...
                    if (nex.has("trading_grounds")) {
                        if (nex.path("trading_grounds").get(0).has("ticker")
                                && nex.path("trading_grounds").get(0).path("ticker").textValue().equals(getDto().getTicket())) {
                            resArr.add(nex);
                        }
                    }
                }
                ...
                log.warn(getDto().getSource() + " не нашла тикер " + getDto().getTicket());
                return null;
            }
        }

        if (!doc.getElementsByClass("mainPrice").isEmpty()) {
        ...
        if (getDto().getCoastList().size() > 0 || getDto().getCostType() != null) {
