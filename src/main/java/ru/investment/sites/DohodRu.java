...
public class DohodRu extends AbstractSite {
    ...
    public ShareDTO task() throws Exception {
        buildUrl(SOURCE_FOUR + getDto().getTicket().toLowerCase());
        Document doc = getDoc();
        if (doc == null) {
            return null;
        }
        ...
        if (doc.getElementsByClass("container").size() > 0) {
            if (doc.getElementsByClass("container").get(0).text().startsWith("404")) {
                return null;
            }
        }
        getDto().setName(doc.html().substring(doc.html().indexOf("leftside-col") + 28).split("</")[0].replaceAll("'", "").replaceAll("\"", ""));
        ...
        if (payDate != null) {
            getDto().setPayDate(LocalDateTime.parse(payDate.split("\\.")[2] + "-" + payDate.split("\\.")[1] + "-" + payDate.split("\\.")[0] + "T12:00:00"));
        }
