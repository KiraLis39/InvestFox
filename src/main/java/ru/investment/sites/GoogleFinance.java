...
public class GoogleFinance extends AbstractSite {
    ...
    public ShareDTO task() {
        ...
        try {
            buildUrl(SOURCE_SIX + getDto().getTicket().toUpperCase() + ":MCX");
            doc = getDoc();
            ...
            try {
                buildUrl(SOURCE_SIX + getDto().getTicket().toUpperCase() + ":NYSE");
                doc = getDoc();
                String someString = doc.text().substring(0, doc.text().indexOf("Stock"));
                if (doc.getElementsByClass("e1AOyf").text().contains("We couldn't find any match for your search")) {
                    throw new Exception();
                ...
                try {
                    buildUrl(SOURCE_SIX + getDto().getTicket().toUpperCase() + ":VIE");
                    doc = getDoc();
                    String someString = doc.text().substring(0, doc.text().indexOf("Stock"));
                    if (doc.getElementsByClass("e1AOyf").text().contains("We couldn't find any match for your search")) {
                        throw new Exception();
                    ...
                    try {
                        buildUrl(SOURCE_SIX + getDto().getTicket().toUpperCase() + ":NASDAQ");
                        doc = getDoc();
                        String someString = doc.text().substring(0, doc.text().indexOf("Stock"));
                        if (doc.getElementsByClass("e1AOyf").text().contains("We couldn't find any match for your search")) {
                            throw new Exception();
                        ...
                        log.error(getDto().getSource() + " не нашла тикер " + getDto().getTicket());
                        return null;
                    }
                }
            }
